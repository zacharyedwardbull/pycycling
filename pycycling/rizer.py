...
class Rizer:
    ...
    async def enable_steering_measurement_notifications(self):
        await self._client.start_notify(rizer_measurement_id, self._steering_measurement_notification_handler)
    ...
    async def set_transmission_rate(self, rate: int):
        ...
        if rate < 0 or rate > 2:
            raise ValueError("Invalid rate: choose 0, 1, or 2 for 8, 16, or 32 Hz respectively")
